...
class QuickSort
{
	...
	private int doPartition(int array[], int startPos, int endPos)
	{
		...
		int pivot = array [ startPos ];//25 | 19 | 2
		...
		array [ i + 1 ] = array [ startPos ];
		array [ startPos ] = tempValue;
		System.out.println("\t--swapping...(i+1)+"+(i+1)+":"+array[i+1]+ " and "+array[startPos]+":endPos:"+startPos);
